        ...
        const int pn, const int cn)
{
    ...
    do {
        ...
        converge = true;                                // Aggregation converge flag
        ...
        #pragma omp parallel
        {
            ...
            for (int i = 0; i < pn; i += 2)
            {
                ...
                // __m128d m_min_dists = _mm_load_pd(min_dists);
                ...
                for (color_t c = 0; c < cn; ++c)
                {
                    ...
                    // double mean_xy[4] = {mean[c].getX(), mean[c].getY(), mean[c + 1].getX(), mean[c + 1].getY()};
                    
                    __m256d m_data_xy = _mm256_load_pd((double*)&data[i]);
                    __m256d m_mean_xy = _mm256_broadcast_pd((__m128d*)(&mean[c]));
                    // __m256d m_mean_xy = _mm256_load_pd((double*)mean_xy);
                ...
                // );
                sums[new_colors[0]] += data[i];
                sums[new_colors[1]] += data[i + 1];
